...
describe("LogoIcon", () => {
  ...
  it("should use default logo URL when no setting is provided", async () => {
    ...
    fetchMock.get("http://localhost//app/assets/img/amara-logo.png", svgContent);
    ...
    const calls = fetchMock.callHistory.calls();
    expect(calls[0].url).toContain("app/assets/img/amara-logo.png");
  });
  ...
  it("should apply textMetabaseBrand when not dark and the logo is the default", () => {
    fetchMock.get("http://localhost//app/assets/img/amara-logo.png", 404);
    setup({}, { "application-logo-url": "app/assets/img/amara-logo.png" });
